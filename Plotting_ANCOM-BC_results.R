...
asv_counts <- anc_full_tax %>% filter(Diff_more_abundant=="F2") %>%
  group_by(Family.y, Comparison2) %>%
...
top_family <- asv_counts %>%
  ...
  dplyr::summarise(total = sum(n)) %>%
  arrange(desc(total)) %>%
  slice(c(1:9, 12:13)) %>% # slice_head(n = 10)
  pull(Family.y)
...
radar_data <- asv_counts %>%
  filter(Family.y %in% top_family) %>%
  pivot_wider(names_from = Family.y, values_from = n, values_fill = 0)
...
summary_table_lolli <- anc_overall %>%
  group_by(Genus, Diff_more_abundant) %>%
  dplyr::summarise(
    n_ASVs = n(),
...
top_lolli$Taxa=paste(top_lolli$Genus, " (",tax$Family[match(top_lolli$Genus, tax$Genus)],")", sep = "" )
